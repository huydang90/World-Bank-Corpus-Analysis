...
# Start the clock!
ptm <- proc.time()
afdb_readability <- with(afdb_corpus_files, readability(text, Year, order.by.readability = FALSE))
# Stop the clock
proc.time() - ptm
```

```{r}
plot(afdb_readability)
```

The African Development Bank (AFDB) reports were downloaded from the internet but were much more difficult to find and access than the World Bank reports that were nicely organized on the same official World Bank website. Although much time was spent trying to find all the AFDB reports, for some reason, only the reports from 2004 until 2018 were available for download online. Indeed, a longer time period would have been preferred for a more direct comparison. However, incorporating the AFDB reports in the analysis is still strengthening the comparison analysis and findings of the paper as the shorter time period allowed for a direct comparison between the World Bank and AFDB reports from 2004 to 2018. 

The AFDB reports showed that the average of the five readability scores is around 19.5 - which is slightly above the average of the 2004-18 World Bank reports averaging around 18.5 - indicating very complex reports that are difficult to understand. The Automated Readability Index and the Gunning Fog Index consistently ranged about 3 points above the average while the SMOG and Coleman Liau scores ranged very close to the average and the Flesch Kincaid ranged about 2 point below the average. The same tendency appears for the World Bank reports in this period. However the different scores are more dispersed for the World Bank reports than for the AFDB reports.